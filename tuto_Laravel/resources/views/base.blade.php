...
<div class="min-h-screen bg-gray-100 min-w-screen">
    ...
    <header class="w-full text-gray-700 bg-white border-t border-gray-100 shadow-sm body-font">
        <div class="container flex flex-col flex-wrap items-center p-5 mx-auto md:flex-row">
            ...
            <nav class="flex flex-wrap items-center text-base lg:w-2/5 md:ml-auto text-gray-500">
                <a href="{{route('blog.index')}}"
                class="  @if(request()->route()->getName()=== 'blog.index')  text-blue-500 @endif
            ...
            <!-- on veut afficher la couleur blue si le lien est active
             @if(request()->route()->getName()=== 'blog.index')  text-blue-500 @endif 
             si nous sommes dans la route dont le nom de la route est.. tu le mets en blue sinon, tu te casses -->
            <a
            ...
            <div>
                <!-- auth permet de savoir si on est connecté -->
                @auth
                     ...
                     <!-- un formulaire pour se déconnecter -->
                @endauth
                <!-- @guest permet de savoir si on est pas connecté-->
                @guest
                    <a href="{{route('auth.login')}}" class="text-gray-900 font-medium hover:text-gray-900">Se connecter</a>
                @endguest
            </div>
        </div>
    </header>

    <section class="container bg-blue-500 px-4 text-white">
        @if(session('success'))
        ...
        @yield("content")
        <!-- on appelle la section crée dans le index.blade -->

        <!-- request nous permet de recupèrer notre requete par tout -->
    </section>
</body>
</html>